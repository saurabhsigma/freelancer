...
export function ThemeZen({ user, projects, config }: ThemeProps) {
    ...
    return (
        <div
            className={`min-h-screen transition-colors duration-700 ease-in-out font-serif ${
                isDark ? "bg-[#121212] text-[#e4e4e4]" : "bg-[#fdfbf7] text-[#1a1a1a]"
            }`}
        >
            {/* Minimal Nav / Theme Toggle */}
            ...
            <div className="max-w-3xl mx-auto px-6 pt-40 pb-24 space-y-32">
                ...
                <header className="space-y-12">
                    <div className="space-y-4">
                        ...
                        <p className="font-sans text-xs uppercase tracking-[0.4em] opacity-50">
                            {user.headline || "Digital Craftsman"}
                        </p>
                    </div>

                    <p className="text-2xl md:text-3xl leading-[1.4] font-light max-w-xl">
                        {user.bio || "Crafting digital experiences with a focus on simplicity and functional elegance."}
                    </p>

                    <div className="flex gap-8 items-center pt-4">
                        <SocialLink href={`mailto:${user.email}`} icon={<Mail size={16} />} label="Email" />
                        {user.socials?.linkedin && <SocialLink href={user.socials.linkedin} icon={<Linkedin size={16} />} label="LinkedIn" />}
                        {user.socials?.github && <SocialLink href={user.socials.github} icon={<Github size={16} />} label="GitHub" />}
                    </div>
                </header>
                ...
                <section className="space-y-24">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="font-sans text-[10px] uppercase tracking-[0.5em] opacity-40">Selected Works</h2>
                    </div>
                                ...
                                <div className="grid md:grid-cols-12 gap-8 items-center">
                                    ...
                                    <div className="md:col-span-5 space-y-6">
                                        <div className="space-y-2">
                                            <span className="font-sans text-[10px] opacity-40">0{idx + 1}</span>
                                            <h3 className="text-3xl font-medium tracking-tight">
                                                {project.title}
                                            </h3>
                                        </div>
                                        <p className="opacity-70 leading-relaxed font-sans text-sm line-clamp-3">
                                            {project.scope}
                                        </p>
                                        ...
                                        {project.liveUrl && (
                                            <Link 
                                                ...
                                                className="inline-flex items-center gap-2 font-sans text-xs uppercase tracking-widest group-hover:translate-x-2 transition-transform duration-300"
                                            >
...
function SocialLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
    return (
        <a 
            href={href} 
            className="flex items-center gap-2 font-sans text-xs uppercase tracking-widest opacity-60 hover:opacity-100 transition-opacity"
            target="_blank"
